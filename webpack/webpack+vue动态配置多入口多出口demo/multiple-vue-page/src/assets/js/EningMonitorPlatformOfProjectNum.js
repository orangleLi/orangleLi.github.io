...
postCall(DeviceNum,function(resultData){
    // debugger
    $("#DeviceNum").text(resultData.results[0].count_device_id);
});
postCall(OnLine,function(resultData){
    // debugger
    $("#OnLine").text(resultData.results[0].count_device_id);
});
setInterval(function () {
    postCall(DeviceNum,function(resultData){
        // debugger
        if($("#DeviceNum").text()!==0 && resultData.results[0].count_device_id!==$("#DeviceNum").text()){
            $("#DeviceNum").text(resultData.results[0].count_device_id);
        }
    });
    postCall(OnLine,function(resultData){
        // debugger
        if($("#OnLine").text()!==0 && resultData.results[0].count_device_id!==$("#OnLine").text()){
            $("#OnLine").text(resultData.results[0].count_device_id);
        }
    });
},3000);
...
var getInfoContentData = {
    ...
    "filters": {
        "new_time": {"__gt": getLastUpDateTime()}
    }
}
postCall(getInfoContentData, function (resultData) {
    console.log(getLastUpDateTime())
    debugger
})
// function initEningMonitorPlatform01(){
//     if(getInfoContentData.Result && getInfoContentData.Code===200) {//处理成功
//         $.each(getInfoContentData.Data, function(index){
//             $(".infoCls:eq("+index+")").text(getInfoContentData.Data[index].ProjectName + " "+ getInfoContentData.Data[index].AlarmDesc + "["+ getInfoContentData.Data[index].AlarmTime+"]");
//         })
//     }
//     else{
//         console.log("处理的结果信息" + getInfoContentData.Message);
//     }
// }
// initEningMonitorPlatform01();
// setInterval(initEningMonitorPlatform01,5000);
    ...
    optionMap = {
            ...
            formatter: function (params) {
                ...
                if(params.value!=undefined){
                    content='<p style="text-align: center;">'
                        +'<div class="dpb" style="padding: 0.02rem 0.05rem;font-family: 微软雅黑;font-size: 0.1rem;color: #ffffff;">'+params.name+'</div>'
                        +'</p>';
                }
                /*设置是否要设置新的提示浮窗的样式*/
        ...
        series: [
            ...
            },
            {
                ...
                coordinateSystem: 'geo',
                opacity: 1,
                data: convertData(earlyingAlarmingData),
                symbolSize: 15,
                label: {
                    normal: {
                        show: false
            ...
            },
            {
                ...
                itemStyle: {
                    normal: {
                        ...
                        borderColor: '#fff',
                        borderWidth: 2,
                        shadowColor: 'rgba(158, 168, 85, 0.6)',
                        shadowBlur: 50
                    },
                    emphasis: {
                        borderColor: '#fff',
    ...
    if (params.componentType === 'geo') {
        if(lastProvinceOrCityName.indexOf('直辖市')===-1){
            if(tag === 0) {
                ...
                provinceOrCityName =params.name;
                $("#info").html(provinceOrCityName);
                tag++;
